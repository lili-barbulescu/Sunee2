...
void PORTB_IRQHandler(void)
{
	...
	// clear IRQ flag
	GPIO_ClearPinsInterruptFlags(GPIOB, GPIO_GetPinsInterruptFlags(GPIOB));
...
void POWER_sleep(uint32_t ms)
{
	...
	if (!APP_CheckPowerMode(SMC_GetPowerModeState(SMC), kAPP_PowerModeStop)) {
		return;
	}
	...
	APP_SetWakeupConfig(kAPP_PowerModeStop);
	// switch power mode (aka go to sleep)
	APP_PowerModeSwitch(SMC_GetPowerModeState(SMC), kAPP_PowerModeStop);
	// restore debug console
	APP_PowerPostSwitchHook(SMC_GetPowerModeState(SMC), kAPP_PowerModeStop);
}
	...
	while (!GPIO_ReadPinInput(BOARD_SW_GPIO, BOARD_SW_PIN)) {
		...
		if (ms_counter > 10000) {
			...
			do {
				...
				APP_PowerPreSwitchHook(curPowerState, kAPP_PowerModeStop);
				...
				_POWER_STATE_ON = 0;
				APP_SetWakeupConfig(kAPP_PowerModeStop);
				APP_PowerModeSwitch(curPowerState, kAPP_PowerModeStop);
				APP_PowerPostSwitchHook(curPowerState, kAPP_PowerModeStop);
